...
class HomePage extends Component {
		...
		books: [
			{
				...
				bookSection: CURRENTLY_READING_ID
			},
			{
				id: 1,
	...
	handleMoveItem = (selectedIdx, bookId) => {
		// this.state.books.find(book => book.id === bookId).bookSection = selectedIdx;
		this.setState(prevState => {
			const newBooks = prevState.books.map(book => {
				if (book.id === bookId) {
					book.bookSection = selectedIdx - 1;
				}
				return book;
			});
		...
		return (
			<div className='list-books'>
				...
				<div className='list-books-content'>
					<div>
						<BookSection
							sectionBooks={this.state.books.filter(
								book => book.bookSection === CURRENTLY_READING_ID
							)}
							sectionTitle='Currently Reading'
							sectionId={CURRENTLY_READING_ID}
							onMoveItem={this.handleMoveItem}
						/>
						<BookSection
							sectionBooks={this.state.books.filter(
								book => book.bookSection === WANT_TO_READ_ID
							)}
							sectionTitle='Want to Read'
							sectionId={WANT_TO_READ_ID}
							onMoveItem={this.handleMoveItem}
						/>
						<BookSection
							sectionBooks={this.state.books.filter(
								book => book.bookSection === READ_ID
							)}
							sectionTitle='Read'
							sectionId={READ_ID}
							onMoveItem={this.handleMoveItem}
